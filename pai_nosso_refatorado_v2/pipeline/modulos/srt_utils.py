...
def ler_srt(caminho: Path | str) -> list[Legenda]:
    ...
    for bloco in blocos:
        ...
        # remove segmentos que são só fragmentos soltos (letra solta, pontuação)
        # ex: YouTube deixa "m" ou "M." sozinhos no último segmento
        if re.fullmatch(r"[A-Za-z]{1,2}[\.,!?]?", texto):
            logger.debug("ler_srt: descartando fragmento solto: %r", texto)
            continue
