...
class Classifier:
    ...
    def __init__(self, config, groq) -> None:
        ...
        # Pasta no Drive — usa config para respeitar estrutura migrada (pai_nosso_refatorado/)
        self.pasta_correcoes = config.pasta_drive_correcoes
        # Pasta onde o pipeline salva os JSONs brutos gerados pelo Groq (backup)
        self.pasta_brutos = config.pasta_drive_brutos
    ...
    def exportar_pacote_revisao(self, legendas_idiomas: dict) -> Path:
        ...
        # Pastas do Drive
        pasta_correcoes_root = self.pasta_correcoes.parent  # .../correcoes/
        pasta_oracao = self.pasta_correcoes                  # .../correcoes/{NOME_ORACAO}
...
`MyDrive/pai_nosso_refatorado_v2/pipeline/correcoes/{nome}/`
